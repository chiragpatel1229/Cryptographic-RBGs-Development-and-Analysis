...
sel_CTR_cipher_name = 'AES128'                          # Select one cipher from this list ('aes128', 'aes192', 'aes256')
...
def strings_xor(a_str, b_str):                              # XOR two byte strings
    x_str = bytes(x ^ y for x, y in zip(a_str, b_str))
    return x_str


def bytes_padding(a_str, total_length_n):                            # Pad or extend a byte string with zeros
    padded_bytes = a_str + b'\x00' * (total_length_n - len(a_str))       # example: extend the Byte String: b'\x01\x02\x03\x00\x00\x00\x00\x00'
    return padded_bytes


def bytes2long_int(a_str):                              # Convert a byte string to a long integer
    long_int = int.from_bytes(a_str, 'big')             # example: b'\x01\x02\x03\x04' = 16909060
    return long_int


def long_int2bytes(a_int):                              # Convert a long integer to a byte string
    integer_2_bytes = a_int.to_bytes((a_int.bit_length() + 7) // 8, 'big')     # example: 16909060 = b'\x01\x02\x03\x04'
    return integer_2_bytes
...
class CTRDRBG:                                      # class for CTR-DRBG mechanism
    ...
    def instantiate(self, entropy=None, additional_data=None):

        if entropy is None:
            entropy = os.urandom(self.seed_length)       # Obtain an entropy source that can provide at least 48 bytes of random bytes
        if additional_data is None:
            additional_data = b'Indian cuisine is good!'            # Provide a personalization string that is unique to your application and 48 bytes long
        ...
        if additional_data:                                    # If data is provided, xor it with the entropy after padding
            '''32 is for without derivation function'''
            if len(additional_data) > 32:                      # 32 is for without derivation function
                raise ValueError(f'Only 32 bytes = 256 bits of data supported.')

            additional_data = bytes_padding(additional_data, 32)
            seed_material = strings_xor(entropy, additional_data)
    ...
    def generate(self, count, additional_data=None):           # Generate random bytes with optional data
        ...
        if additional_data:                                    # if the data is provided by the user
            if len(additional_data) > 32:        # Check the length of the data
                raise ValueError(f'Additional data is more than required! It should be less than 32 byts for without derivation!!')

        else:                                       # if data is not provided then Use the internally provided data
            additional_data = b'A user can add here anything. Based on NIST standards'  # it can be any length, later used padding in the next step
            additional_data = bytes_padding(additional_data, 32)

        self.key, self.V = self.update(additional_data, self.key, self.V)
        ...
        self.key, self.V = self.update(additional_data or b'', K, V)  # Update the key and the counter with the data or zeros
    ...
    def reseed(self, additional_data=None):                    # Reseed the DRBG with entropy and optional data
        ...
        if additional_data:                                    # Check the length of the data
            if len(additional_data) > self.seed_length:
                raise ValueError(f'Additional data is more than required! It should be less than 32 byts for without derivation!!')

        else:                                       # Use internal data if data is not provided
            '''32 is for without derivation function'''
            additional_data = b'A user can add here anything.'  # it can be any length, used padding in the next step
            additional_data = bytes_padding(additional_data, 32)

        if len(entropy) != self.seed_length:
            raise ValueError(f'Too much entropy. it should be {self.seed_length}')

        seed_material = strings_xor(entropy, bytes_padding(additional_data, self.seed_length))  # If data is provided or not, xor it with the entropy after padding
