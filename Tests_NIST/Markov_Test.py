...
If the entropy source is used for cryptographic applications, the min-entropy per symbol(output) should be at least 0.95 or higher.
The p-hat_max value should be less or equal to 0.5 => lower the value higher the unpredictability and very high entropy of the RBG.
...
def markov_test(data, print_output: int = 0):
    ...
    assert s_len > 1                            # Less than 2 symbols don't make sense for a Markov model.

    # Get counts for unconditional and transition probabilities as in section 6.3.3.2
    for i in range(s_len - 1):
        if data[i] == 0:                        # raise the counter if the bit is 0
            C_0 += 1
            if data[i + 1] == 0:                # raise the counter if the bit is 00
                C_00 += 1
        elif data[i + 1] == 0:                  # raise the counter if the bit is 10
            C_10 += 1

    # C_0 is now the number of 0 bits from S[0] to S[len-2]
    C_1 = s_len - 1 - C_0                       # C_1 is the number of 1 bits from S[0] to S[len-2]
