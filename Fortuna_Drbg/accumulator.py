...
class Accumulator:
    pool_number = 32
    min_pool_size = 64    # Minimum length of the pool
    reseed_interval = 0.1
    P = []

    def __init__(self):
        self.P = [b''] * self.pool_number
        self.ReseedCnt = 0      # Counter for regenerating seeds
        self.generator = Generator()    # Initialize generator
        self.last_seed = time.time()

    # Generate pseudo-random numbers
    def random_data(self, n):
        # n The number of bytes of pseudo-random numbers to be generated
        # Return pseudo-random byte string
