...
class QuantumRNG(object):
    ...
    def _update(self):
        ...
        if len(self.entropy) < 512:  # use the system entropy bcz the generating quantum bits takes time
            sel_int = self.sec_.randrange(2 * 10 ** 100,
                                          3 * 10 ** 100)  # select a random number between the given range
        ...
        if len(self.entropy) < 1025:  # our major focus is on adding quantum bits to the seed value
            quantum_bits = threading.Thread(target=self.Refill_entropy,
                                            name='GenCj bitsP')  # using thread as this process takes unexpected time
        ...
        self.seed += int(self.entropy[:256],
                         2)  # convert only first 256 bits to integer and make addition with the seed value to update it
    ...
    def Quantum_circuit(self):
        ...
        # 1.4.3 Collect the measured bits from the quantum bit generation process ======================================
    ...
    def ran_bytes(self, N=0):
        ...
        b_str = int_v.to_bytes(num_bytes,
                               byteorder='big')  # Convert an integer to byte string using the inbuilt function
...
for _ in range(0, 30):
    random_bits = quantum_rng.ran_bits(
        n_req_bits=number_of_bits)  # This function can be used in a loop for constant bit production
